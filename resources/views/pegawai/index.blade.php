            ...
            <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ route('pegawai.create') }}"
                        class="inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors bg-blue-600 rounded-base hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 shadow-sm">
                ...
                <form method="GET" action="{{ route('pegawai.index') }}" class="relative w-full sm:w-64 lg:w-96">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}"
                        class="block w-full p-2.5 pl-10 text-sm text-gray-900 border border-gray-300 rounded-base bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Cari Nama, NUPTK, Email..." />
                </form>
            </div>
                    ...
                    <thead class="text-sm text-body bg-neutral-secondary-soft border-b rounded-base border-default">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 font-medium">
                                <a href="{{ route('pegawai.index', array_merge(request()->query(), ['sort' => 'name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                                    class="flex items-center group">
                                    Pegawai
                                    <svg class="w-3 h-3 ml-1.5 opacity-50 group-hover:opacity-100 transition-opacity"
                            ...
                            <th scope="col" class="px-6 py-3 font-medium">
                                <a href="{{ route('pegawai.index', array_merge(request()->query(), ['sort' => 'nuptk', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                                    class="flex items-center group">
                                    NUPTK
                                    <svg class="w-3 h-3 ml-1.5 opacity-50 group-hover:opacity-100 transition-opacity"
                                        fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr
                                ...
                                <th scope="row" class="px-6 py-4 flex items-center whitespace-nowrap">
                                    @if ($peg->foto)
                                        <img class="w-10 h-10 rounded-full object-cover border border-gray-200"
                                            src="{{ asset('storage/' . $peg->foto) }}" alt="{{ $peg->name }}">
                                    @else
                                        <div
                                            class="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600 border border-gray-200">
                                            <span
                                                class="font-medium text-gray-600 dark:text-gray-300">{{ substr($peg->name, 0, 2) }}</span>
                                        </div>
                                    @endif
                                    <div class="pl-3">
                                        <a href="{{ route('pegawai.show', $peg->id) }}"
                                            class="text-base font-semibold text-heading hover:text-blue-600 transition-colors">{{ $peg->name }}</a>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('pegawai.show', $peg->id) }}"
                                            class="p-2 text-gray-600 bg-gray-50 rounded-base hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600 transition-colors"
                                            title="Lihat Profil">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-body-subtle">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="mt-1 text-sm">Coba ubah kata kunci pencarian atau tambah data baru.
                                        </p>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 space-y-4 md:p-5">
                    <form action="{{ route('importpegawai') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">File Excel Harus sesuai
                            dengan format export excel</p>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            for="small_size">Import File Excel Anda</label>
                        <input accept=".xlsx, .csv, .xls" name="file"
                            class="block w-full mb-5 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                            id="small_size" type="file">
                        <button type="submit"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 w-full">Unggah</button>
                    </form>
                </div>
            </div>
        </div>
