<x-layout.layout>
    ...
    <div class="p-4 mt-5 border-2 border-gray-200 rounded-lg dark:border-gray-700">
        ...
        <form action="{{ isset($pemetaan) ? route('kelassiswa.update', $pemetaan->id) : route('kelassiswa.store') }}"
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    ...
                    <select id="tahun" name="tahun"
                        ...
                        <option value="" disabled {{ old('tahun',isset($pemetaan)?$pemetaan->
                            tahun_id:'')==''?'selected': '' }}>Tahun
                            Ajaran</option>
                        @foreach($tahun as $key=>$t)
                        <option value="{{ $t->id }}" {{ old('tahun',isset($pemetaan)?$pemetaan->tahun_id:'')==$t->id ?
                            'selected' : ''
                ...
                <div>
                    ...
                    <select id="kelas" name="kelas"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="" disabled {{ old('kelas',isset($pemetaan)?$pemetaan->
                            kelas_id:'')==''?'selected':'' }}>Kelas
                        </option>
                        @foreach($kelas as $key=>$k)
                        <option value="{{ $k->id }}" {{ old('kelas',isset($pemetaan)? $pemetaan->
                ...
                <div>
                    ...
                    <select id="kelas" name="siswa"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="" disabled {{ old('siswa',isset($pemetaan)?$pemetaan->
                            siswa_id:'')==''?'selected':'' }}>Siswa
                        </option>
                        @foreach($siswa as $key=>$s)
                        <option value="{{ $s->id }}" {{ old('siswa',isset($pemetaan)? $pemetaan->
            ...
            <button type="submit"
                ...
                isset($pemetaan) ? 'Update' : 'Simpan' }}</button>
        </form>
        ...
        {{-- Tabel --}}
        <form id="MyForm" action="{{ route('kelassiswa.destroy',$pemetaans->first()->id ?? '') }}" method="POST">
            @csrf @method('delete')

            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <div
                    class="flex flex-wrap items-center justify-between pb-4 space-y-4 flex-column sm:flex-row sm:space-y-0 me-4 ms-3">
                    <div class="flex items-center space-x-2">
                        ...
                        <small class="inline-block text-gray-500 dark:text-gray-400">Items per page</small>
                    </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">
                                <div class="flex items-center">NIPD
                                    <a
                                        href="?sort=nipd&direction={{ request('direction') === 'asc' ? 'desc' : 'asc' }}"><svg
                                            class="w-3 h-3 ms-1.5 {{ request('sort') == 'nipd' ? (request('direction') == 'asc' ? 'rotate-180' : '') : '' }}"
                    ...
                    <tbody>
                        ...
                        <tr
                            ...
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input id="checkbox" type="checkbox" name="id[]" value="{{ $pemetaan->id }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('kelassiswa.edit',$pemetaan->id) }}"
    ...
    <div class="mt-4">
        {{ $pemetaans->appends(['per_page' => request('per_page'), 'search' => request('search')])->links() }}
    </div>
    </div>
                ...
                <div class="p-4 space-y-4 md:p-5">
                    <form action="{{ route('kelas-siswa-import') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                            Proses ini menginput siswa ke dalam setiap kelas sehingga data siswa dan kelas harus valid
                        </p>
