<x-layout.layout>
    ...
    <div class="p-4 mt-5 border-2 border-gray-200 rounded-lg dark:border-gray-700">
        ...
        <form action="{{ isset($pemetaan) ? route('kelassiswa.update', $pemetaan->id) : route('kelassiswa.store') }}"
            class="hidden" id="InputSiswa" method="POST">
            @csrf
            @isset($pemetaan)
                @method('PUT')
            @endisset
        ...
        {{-- Tabel --}}
        {{-- <form id="MyForm" action="{{ route('kelassiswa.destroy', $pemetaans->first()->id ?? '') }}"
            method="POST">
            @csrf @method('delete') --}}
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <div
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('kelassiswa.index') }}" method="GET" id="searchForm">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">
                            <div class="flex items-center">NIPD
                                <a href="?sort=nipd&direction={{ request('direction') === 'asc' ? 'desc' : 'asc' }}"><svg
                ...
                <tbody>
                    @foreach ($pemetaans as $pemetaan)
                        <tr
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('kelassiswa.edit', $pemetaan->id) }}" onclick="muncul()"
    ...
    <script>
        ...
        //     // Hapus input _method jika ada (agar jadi mode tambah)
        ...
        //     // Set ulang select ke default
        //     form.querySelectorAll('select').forEach(select => {
        //         select.selectedIndex = 0;
        //     });

        //     // Hapus value di input text manual jika tidak ikut reset
