<x-layout.layout>
    <x-breadcrumb :title="'Profil Pegawai'" :breadcrumbs="[
        ['name' => 'Home', 'href' => route('dashboard.index')],
        ['name' => 'Data Pegawai', 'href' => route('pegawai.index')],
        ['name' => $pegawai->name, 'href' => '#'],
    ]" />
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    <div class="-mt-12 relative">
                        <div class="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white dark:border-gray-800 bg-white dark:bg-gray-700 overflow-hidden shadow-md">
                            @if($pegawai->foto)
                                <img src="{{ Storage::url($pegawai->foto) }}" alt="{{ $pegawai->name }}" class="w-full h-full object-cover">
                            @else
                    ...
                    <div class="flex-1 pt-6 md:pt-2">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $pegawai->name }}</h1>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                                    <span>{{ $pegawai->nuptk }}</span>
                                    <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                                    <span>{{ $pegawai->jenisptk ?? 'Pegawai' }}</span>
                                    <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700">
                <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist">
                    <li class="me-2" role="presentation">
                        <button class="inline-block p-4 border-b-2 rounded-t-lg" id="profile-tab" data-tabs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="true">Detail Identitas</button>
                    </li>
                    <li class="me-2" role="presentation">
            ...
            <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 space-y-6">
                            ...
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">
                                        {{ $pegawai->kotalahir }}, {{ $pegawai->tanggallahir ? \Carbon\Carbon::parse($pegawai->tanggallahir)->isoFormat('D MMMM Y') : '-' }}
                                    </dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Jenis Kelamin</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">
                                        {{ $pegawai->jk == 'L' ? 'Laki-laki' : ($pegawai->jk == 'P' ? 'Perempuan' : '-') }}
                                    </dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Agama</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->agama }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Status Pernikahan</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->kawin }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Email</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->email ?? '-' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">No. HP</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->hp ?? '-' }}</dd>
                                </div>
                                <div class="md:col-span-2">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Alamat</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">
                                        {{ $pegawai->alamat }}, RT {{ $pegawai->rt }}/RW {{ $pegawai->rw }}
                                    </dd>
                                </div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Informasi Kepegawaian</h3>
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">NUPTK</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->nuptk }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Jenis PTK</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->jenisptk }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">SK Pengangkatan</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->skpengangkatan }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Lembaga Pengangkat</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->lembagapengangkatan }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Pangkat / Golongan</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->PangkatGolongan }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Sumber Gaji</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white font-semibold">{{ $pegawai->sumbergaji }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Keluarga</h3>
                            <ul class="space-y-4">
                                <li class="text-sm">
                                    <span class="block text-gray-500">Ibu Kandung</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->ibukandung ?? '-' }}</span>
                                </li>
                                <li class="text-sm">
                                    <span class="block text-gray-500">Pasangan (Suami/Istri)</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->suamiistri ?? '-' }}</span>
                                </li>
                                <li class="text-sm">
                                    <span class="block text-gray-500">Pekerjaan Pasangan</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->pekerjaansuamiIstri ?? '-' }}</span>
                                </li>
                            </ul>
                        </div>
                         
                        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Administrasi</h3>
                            <ul class="space-y-4">
                                <li class="text-sm">
                                    <span class="block text-gray-500">NPWP</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->npwp ?? '-' }}</span>
                                </li>
                                <li class="text-sm">
                                    <span class="block text-gray-500">NIK (KTP)</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->nonik ?? '-' }}</span>
                                </li>
                                <li class="text-sm">
                                    <span class="block text-gray-500">No. Kartu Keluarga</span>
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $pegawai->nokk ?? '-' }}</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                            ...
                            @forelse($recentLogs as $log)
                                <li class="mb-6 ml-6">
                                    ...
                                    <h3 class="flex items-center mb-1 text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ $log->status }}
                                        @php
                                            $statusColor = match (trim($log->status)) {
                                    ...
                                    <time class="block mb-2 text-xs font-normal leading-none text-gray-400 dark:text-gray-500">{{ \Carbon\Carbon::parse($log->tanggal)->isoFormat('dddd, D MMM Y') }}</time>
                                    <p class="mb-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                                        Masuk: {{ $log->jam_masuk ?? '-' }} | Pulang: {{ $log->jam_pulang ?? '-' }}
                                    </p>
                                </li>
                            @empty
                                <li class="ml-6">
